...
struct DroneCommand {
  int d; char type;
  union {
    struct { int w; int p; int n; } L, U;
    struct { int o; int p; int n; } D;
    struct { int d; } W;
  } spec;
};
...
void LoadProblem(const string& path) {
  ...
  fin >> W >> H >> Nd >> T >> M;
  assert(W >= 1);  assert(W <= 100000);
  assert(H >= 1);  assert(H <= 100000);
  assert(Nd >= 1); assert(Nd <= 1000);
  assert(T >= 1);  assert(T <= 1000000);
  assert(M >= 1);  assert(M <= 10000);

  fin >> Np;
  assert(Np >= 1); assert(Np <= 10000);
  product_weight.resize(Np);
  for (int p = 0; p < Np; p++) {
    fin >> product_weight[p];
    assert(product_weight[p] >= 1); assert(product_weight[p] <= M);
  }
  
  fin >> Nw;
  assert(Nw >= 1); assert(Nw <= 10000);
  warehouse_loc.resize(Nw);
  warehouse_stock.resize(Nw);
  for (int w = 0; w < Nw; w++) {
    int x, y; fin >> x >> y; warehouse_loc[w] = {x, y};
    assert(x >= 0); assert(x < W);
    assert(y >= 0); assert(y < H);
    for (int p = 0; p < Np; p++) {
      int stock; fin >> stock;
      assert(stock >= 0); assert(stock <= 10000);
      warehouse_stock[w][p] = stock;
    }
  }

  fin >> No;
  assert(No >= 1); assert(No <= 10000);
  order_loc.resize(No);
  order_request.resize(No);
  for (int o = 0; o < No; o++) {
    int x, y; fin >> x >> y; order_loc[o] = {x, y};
    assert(x >= 0); assert(x < W);
    assert(y >= 0); assert(y < H);
    int num_items; fin >> num_items;
    assert(num_items >= 1); assert(num_items <= 10000);
    while (num_items--) {
      int p; fin >> p;
      assert(p >= 0); assert(p < Np);
      order_request[o][p]++;
    }
  }
...
void LoadSolution(const string& path) {
  ...
  int Q; fin >> Q;
  assert(Q >= 0); assert(Q <= Nd * T);
  cout << "  Loading " << Q << " commands." << endl;
  const set<char> valid_commands = {'L', 'U', 'D', 'W'};
  while (Q--) {
    int d; fin >> d;
    assert(d >= 0); assert(d < Nd);
    char type;
    { string ts; fin >> ts; assert(ts.size() == 1); type = ts[0]; }
    assert(valid_commands.count(type) == 1);
    DroneCommand cmd { .d = d, .type = type };
    switch (type) {
      case 'L': {
        fin >> cmd.spec.L.w >> cmd.spec.L.p >> cmd.spec.L.n;
        assert(cmd.spec.L.w >= 0); assert(cmd.spec.L.w < Nw);
        assert(cmd.spec.L.p >= 0); assert(cmd.spec.L.p < Np);
        assert(cmd.spec.L.n >= 1);
        break;
      } 
      case 'U': {
        fin >> cmd.spec.U.w >> cmd.spec.U.p >> cmd.spec.U.n;
        assert(cmd.spec.U.w >= 0); assert(cmd.spec.U.w < Nw);
        assert(cmd.spec.U.p >= 0); assert(cmd.spec.U.p < Np);
        assert(cmd.spec.U.n >= 1);
        break;
      }
      case 'D': {
        fin >> cmd.spec.D.o >> cmd.spec.D.p >> cmd.spec.D.n;
        assert(cmd.spec.D.o >= 0); assert(cmd.spec.D.o < No);
        assert(cmd.spec.D.p >= 0); assert(cmd.spec.D.p < Np);
        assert(cmd.spec.D.n >= 1);
        break;
      }
      case 'W': {
        fin >> cmd.spec.W.d;
        assert(cmd.spec.W.d >= 1); assert(cmd.spec.W.d <= T);
      }
    }
...
int next_relevant_moment() {
  ...
  if (!delivery_schedule.empty()) res = min(res, delivery_schedule.begin()->first); 
...
int command_duration(const DroneCommand& cmd) {
  ...
  switch (cmd.type) {
    case 'L': { dest_loc = warehouse_loc[cmd.spec.L.w]; break; }
    case 'U': { dest_loc = warehouse_loc[cmd.spec.U.w]; break; }
    case 'D': { dest_loc = order_loc[cmd.spec.D.o]; break; }
    case 'W': return cmd.spec.W.d;
  }
...
void Simulate() {
  ...
  while ((t = next_relevant_moment()) >= 0) {
    ...
    for (const auto& event : drone_schedule) {
      ...
      auto cmd = drone_commands[d].front(); drone_commands[d].pop();
