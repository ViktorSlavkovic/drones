...
class GaSolver : public ProblemSolver {
 ...
 protected:
  ...
  // order.
  const std::vector<std::vector<int>> closest_ws_;
  // Product weights map as required by the splitter.
  const std::map<int, int> product_weights_;
  // The alloc - which warehouse gives what exactly to which order.
  const util::Allocator::Alloc alloc_;
  // Enumerates the implemented strategies to make it easier to call them by
  // number.
  const std::vector<std::function<bool(int)>> strategy_;

  // Calculates closest warehouses for each order (see closest_ws_).
  static std::vector<std::vector<int>> CalcClosestWarehouses(
      const Problem& problem);
  // Fills in the product weights map (see product_weights_).
  ...
  // strategies. Used to initialize strategy_. 
  static std::vector<std::function<bool(int)>> FilterStrategies(
      GaSolver* ga_solver);
  ...
  std::vector<int> drone_location_;
  // Last moment at which the drone is busy.
  std::vector<int> drone_busy_until_;
  // For each warehouse, it stores what it has or will have for a certain order.
  // For each (w, o) pair it stores additions to the warehouse mapped by the
  ...
  // Structure: [o][w][p] -> n
  // Keep compact!
  std::map<int, std::map<int, std::map<int, int>>> pending_orders_;
  // Total pending weight for each order.
  std::vector<int> pending_orders_weight_;
  // Maps drones to their ongoing transactions.
  std::map<int, CommandTransactionData> transactions_;
  // The solution being built.
  std::unique_ptr<Solution> solution_;
  // Latest delivery time for each order.
  std::vector<int> latest_delivery_time_;
  // Score of the solution being build.
  int score_;

  // Initializes the core solver variables.
  void InitSolution();
  // Returns pair (int - wait time, std::map<int, int> inventory map) which says
  // what can be taken upon arrival and how much do we need to wait.
  ...
  // Gives the specified inventory (what) to the specified warehouse (w) for the
  // specified order (o). This doesn't generate any commands, it just does the
  // book-keeping. It's assumed that what is the same what that has been taken
  // before by Take().
  void Give(int arrival, int w, int o, const std::map<int, int>& what);
  // Delivers the specified inventory (what) to the specified order (o) from the
  // specified warehouse (w). This doesn't generate any commands, it just does
  // the book-keeping. It's assumed that what is the same what that has been
  // taken befire by Take() and that what is a subset of what is left for that
  // order. Returns true if the order gets completed.
  ...
  // CommandTransactionStart(d). Dies if there isn't a transaction happening for
  // d. This ends the transaction.
  ...
  // transatction - a transaction is started and the return value tells whether
  // it's successfully commited (true) or rolled-back (false).
  ...
  //    that the inter-warehouse distance is minimal. The warehouses have to be
  //    different.
  ...
  // Creates a solution using the provided drone strategies. For each drone,
  // there's a vector of strategies that are executed circularly.
  void PackSolution(const std::vector<std::vector<int>>& drone_strategies,
                    bool log_success_ratios = false);

  // Generates a single individual in the GA's population.
  std::vector<std::vector<bool>> GenerateIndividual(int log_num_strategies);
  // Do a single evaluation.
  int Eval(const std::vector<std::vector<bool>>& individual,
           int log_num_strategies, bool log_success_ratios = false);
